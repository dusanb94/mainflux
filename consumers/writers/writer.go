...
func Start(sub messaging.Subscriber, consumer consumers.MessageConsumer, transformer transformers.Transformer, subjectsCfgPath string, logger logger.Logger) error {
	w := writer{
		consumer:    consumer,
	...
	for _, subject := range subjects {
		if err := sub.Subscribe(subject, w.handler); err != nil {
			return err
		}
	}
	return nil
...
func (c *writer) handler(msg messaging.Message) error {
	t, err := c.transformer.Transform(msg)
	if err != nil {
		return err
	}

	return c.consumer.Consume(t)
}
